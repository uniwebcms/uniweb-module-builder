/**
 * A alt version of server and tunnel that doesn't use the
 * cloudflared library.
 */
...
if (!fs.existsSync(absoluteDestPath)) {
    ...
    if (!fs.existsSync(absoluteDestPath)) {
        console.error(`Error: The specified dest folder does not exist: ${absoluteDestPath}`);
        process.exit(1);
    }
}
...
const httpServer = spawn('npx', [
    ...
    '--gzip',
    '-c' + cacheTime,
    // '-P', // Proxy to a URL for unmatched requests
...
if (process.env.TUNNEL_URL) {
    ...
    cloudflared.on('error', (error) => {
        if (error.code === 'ENOENT') {
            console.error(
                `Error: Command '${cloudflaredExecutable}' not found. Please make sure it's installed and in your PATH.`
            );
        } else {
